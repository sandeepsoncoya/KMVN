...
public function loadhoteldetails(){
        ...
        if (!empty($this->data)) {
            ...
            if (isset($this->data[$modelName])) {
                ...
                if($this->$modelName->save($this->data[$modelName])) {
                    ...
                    $postData['phone']             =  $bookingData['ActivityBookings']['phone'];
    ...
    function add_extra_bed(){
        ...
        if($this->request->is('ajax')){
            ...
            $no_of_rooms = $roomArray[$bedId]['no_of_rooms'];
            $night = $this->data['night'];
            $extraBed = $this->data['value'];
            if($extraBed ==0){
                $extraBedCharge =  ($roomRate/100)*20;
                $totalExtraBedChnage =  $extraBedCharge * $night * $no_of_rooms;
                $response['status']=false;
    ...
    public function bookingList(){
        ...
        $cond = [ 'RoomReservation.trans_date != ' => '', 'RoomReservation.user_id' => $customer_id ];

        if (isset($this->request->data) && !empty($this->request->data)) {

            $draw           = !empty($this->request->data['draw']) ? $this->request->data['draw'] : 0;
            $row            = !empty($this->request->data['start']) ? $this->request->data['start'] : 0;
            $rowperpage     = !empty($this->request->data['length']) ? $this->request->data['length'] : 0; // Rows display per page
            $columnIndex    = !empty($this->request->data['order'][0]['column']) ? $this->request->data['order'][0]['column'] : 0; // Column index

            
            if($this->request->data['columns'][$columnIndex]['data'] == 'sr'){
                $columnName     = 'RoomReservation.id'; // Column name
            }else if($this->request->data['columns'][$columnIndex]['data'] == 'title'){
                $columnName     = 'Hotel.id';
            }else{
                $columnName     = 'RoomReservation.'.$this->request->data['columns'][$columnIndex]['data']; // Column name
            }
            
            $columnSortOrder= $this->request->data['order'][0]['dir']; // asc or desc
            

            if(!empty($this->request->data['search']['value'])){
                $cond[] = "RoomReservation.booking_id LIKE '%".trim($this->request->data['search']['value'])."%'";
            }
            
            if(!empty($this->request->data['created'])){
                $createdDate = date('Y-m-d', strtotime($this->request->data['created']));
                $cond[] = [ 'DATE(RoomReservation.created)' => $createdDate ];
            }
            
            if(!empty($this->request->data['check_in'])){
                $checkInDate = date('Y-m-d', strtotime($this->request->data['check_in']));
                $cond[] = [ 'DATE(RoomReservation.check_in)' => $checkInDate ];
            }
        }else{
            $columnName = 'RoomReservation.id';
            $columnSortOrder = 'Desc';
        }

        $totalRecords = $this->RoomReservation->find('count',[ 'group'=>['RoomReservation.booking_id'] ]);
        
        $totalRecordwithFilter = $this->RoomReservation->find('count',[ 'conditions'=> $cond, 'group'=>['RoomReservation.booking_id'] ]);
        //pr($totalRecordwithFilter); die;
        ...
        $room_reservations = $this->RoomReservation->find('all',[ 'conditions'=> $cond, 'group'=>['RoomReservation.booking_id'], 'limit' => $rowperpage, 'offset'=> $row, 'order'=> $order, 'order'=> $order ]);
        ...
        foreach($room_reservations as $reservation){
            $data[$i]['sr'] = $sr;
            $data[$i]['booking_id'] = $reservation['RoomReservation']['booking_id'];
            $data[$i]['created'] = date('d-M-Y D', strtotime($reservation['RoomReservation']['created']));
            $data[$i]['title'] = $reservation['Hotel']['title'];
            $data[$i]['check_in'] = date('d-M-Y D', strtotime($reservation['RoomReservation']['check_in']));
            $data[$i]['action'] = '<a href="'.$siteUrlfront.'room_reservation/viewBookings/'.$reservation['RoomReservation']['booking_id'].'">view booking</a>';
            $i++;
            $sr++;
        }
    ...
    public function walletList(){
        ...
        $siteUrlfront = Configure::read('siteUrlfront');
        $customer_id = $this->Session->read('CustomerData.Customers.id');
        $cond = [ 'UserWallets.company_user_id' => $customer_id ];
        ...
        if (isset($this->request->data) && !empty($this->request->data)) {
            ...
            if($this->request->data['columns'][$columnIndex]['data'] == 'sr'){
                $columnName     = 'UserWallets.id'; // Column name
            }else{
                $columnName     = 'UserWallets.'.$this->request->data['columns'][$columnIndex]['data']; // Column name
            }
            ...
            if(!empty($this->request->data['search']['value'])){
                $cond[] = "UserWallets.description LIKE '%".trim($this->request->data['search']['value'])."%'";
            }
            
            if(!empty($this->request->data['created'])){
                $createdDate = date('Y-m-d', strtotime($this->request->data['created']));
                $cond[] = [ 'DATE(UserWallets.created)' => $createdDate ];
            }

            if(!empty($this->request->data['amount_type'])){
                $cond[] = [ 'UserWallets.amount_type' => $this->request->data['amount_type'] ];
            }
            
        }else{
            $columnName = 'UserWallets.id';
            $columnSortOrder = 'Desc';
        }
        ...
        $totalRecords = $this->UserWallets->find('count',[ 'conditions'=> $cond ]);
        
        $totalRecordwithFilter = $this->UserWallets->find('count',[ 'conditions'=> $cond ]);

        $user_wallets = $this->UserWallets->find('all',[ 'conditions'=> $cond, 'limit' => $rowperpage, 'offset'=> $row, 'order'=> $order ]);
        ...
        foreach($user_wallets as $wallet){
            $data[$i]['sr'] = $sr;
            $data[$i]['amount_type'] = $wallet['UserWallets']['amount_type'];
            $data[$i]['amount'] = $wallet['UserWallets']['amount'];
            $data[$i]['description'] = $wallet['UserWallets']['description'];
            $data[$i]['created'] = date('d-M-Y D', strtotime($wallet['UserWallets']['created']));
            $sr++;
            $i++;
    ...
    public function view_bookings(){
        $this->layout = false;
            ...
            $bookings = $this->RoomReservation->find('all',['conditions'=>['RoomReservation.booking_id'=>$this->request->data['booking_id']]]);

            $result = array();
            foreach ($bookings as $element) {
                $result[$element['RoomReservation']['booking_id']][] = $element;
            }
            $this->set('bookings',$result);
        ...
        public function cancel_booking(){
            ...
            $roomReservation = $this->RoomReservation->find('first',['conditions'=>['RoomReservation.booking_id'=>$this->request->data['booking_id']]]);

            $roomCancellation = $this->RoomCancellation->find('all',['conditions'=>['RoomCancellation.bed_type_id'=>$this->request->data['bed_id'],'RoomCancellation.room_id'=>$this->request->data['room_id']]]);
            
            $this->set('roomReservation',$roomReservation);
            $this->set('roomCancellation',$roomCancellation);

            $this->render('cancel_booking'); 
