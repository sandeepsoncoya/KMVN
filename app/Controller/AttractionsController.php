 ...
 */
    ...
    public function index(){
       $destinations =  $this->Attraction->find('all',['conditions'=>['Attraction.is_active'=>1],'order'=>['created'=>'DESC'], 'limit' => 2]);

       $destinationsAll =  $this->Attraction->find('all',['conditions'=>['Attraction.is_active'=>1],'order'=>['created'=>'DESC']]);
	...
	public function details() {
        $this->Attraction->bindModel([
                    'hasMany'=>[
                        'AttractionImages'=>[
                          'className'=>'Admin.AttractionImages',
                          'foreignKey'=>'attraction_id'
                        ]
                    ],
                    
                ]);
        $this->Attraction->recursive = 2;
        $attractionDetails = $this->Attraction->findBySlug($this->request->params['pass'][0]);
        $this->set(compact('attractionDetails'));
