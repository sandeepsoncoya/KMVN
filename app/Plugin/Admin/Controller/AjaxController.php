...
class AjaxController extends AdminAppController
{
    ...
    public function add_bed_type(){
        if($this->request->is('ajax')){
            ...
            if(isset($this->request->data['bedId'])){
                $this->loadModel("Admin.".$modelName);
                $data =  $this->$modelName->findById($this->request->data['bedId']);
                $bedTypeImages = $this->RoomImages->find('all',['conditions'=>['bed_type_id'=>$this->request->data['bedId']]]);
                $this->request->data[$modelName] = $data[$modelName]; 
            }
           $this->set(compact('modelName','roomId','bedType','bedTypeImages'));
    ...
    public function list_bed_type_hotel(){
        $this->autorender = false;
        if ($this->request->is('ajax')) {
            if (!empty($this->data)) {
               $this->layout = 'ajax';
                ...
                $bedTypeData = $this->BedTypeHotel->find('all',['conditions'=>['room_id'=>$roomId]]);
               
              // pr($bedTypeData); die;
                
            }
            $this->set(compact('bedTypeData'));
            $this->render('Ajax/bed_type_list'); 
